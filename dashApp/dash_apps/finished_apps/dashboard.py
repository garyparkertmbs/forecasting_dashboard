...
## Callback for storing CSV into prelim store
@app.callback(Output('prelim-store', 'data'),
...
def set_dropdown(prelim_stored_data):
    ...
    df1 = pd.read_json(prelim_stored_data, orient='split')
    product_options = []
    for po in df1['Item'].unique():
        product_options.append({'label': str(po), 'value': po})
...
@app.callback(Output('fcast_graph_main', 'figure'),
            ...
            State('input_fcast_period', 'value'), prevent_initial_call=True)

def update_figure(prelim_stored_data, stored_data, button_click, product_dropdown, value):

    if [prelim_stored_data, stored_data, button_click, product_dropdown, value] == [None, None, None, None, None]:
        return dash.no_update

    # changed_id = [p['prop_id'] for p in callback_context.triggered][0]
    if button_click:
        print("stored_data", stored_data)
        df1 = pd.read_json(stored_data['df1'], orient='split')
        print(df1.head())
...
def clean_data(prelim_store_data, n_clicks, value): # TO BE UPDATED! Need to update the value to feed in user input value for Forecast Period
    changed_id = [p['prop_id'] for p in callback_context.triggered][0]
    if 'button_fcast' in changed_id:
        
        df_out = pd.read_json(prelim_store_data, orient='split')

        print(df_out.head())

        df_results, df_results_best, df1_f1 = run_fcast_full(df_out, value)
        ...
        return { 'df1' : df_1.to_json(date_format='iso', orient='split'), 'df2' : df_2.to_json(date_format='iso', orient='split'), 'df3' : df_3.to_json(date_format='iso', orient='split')}
...
def update_table1( stored_data):
    if stored_data == None:
        raise PreventUpdate
    df = pd.read_json(stored_data['df2'], orient='split')
...
def table_fcast_detail(stored_data, product_dropdown):
    if stored_data == None:
        raise PreventUpdate
    df = pd.read_json(stored_data['df1'], orient='split')
